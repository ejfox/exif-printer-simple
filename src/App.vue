      ...
      <div class="w-64 border-r card-subtle flex-shrink-0 overflow-y-auto">
        ...
        <div v-if="photos.length > 0" class="p-4 space-y-3">
          ...
          <div class="space-y-1">
            ...
            <div class="text-micro text-gray-400 mt-1">
              Legacy control — use Advanced Margin Controls for precision
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    computeLayout(sizeConfig) {
      ...
      const textBandBaseline = Math.max(fontSize, margins.textBleed)
      const minTextBand = textBandBaseline + textInset

      const imageMargins = {
        top: Math.max(margins.top, minTextBand),
        bottom: Math.max(margins.bottom, minTextBand),
        left: Math.max(margins.left, textInset),
        right: Math.max(margins.right, minTextBand)
      }

      const imageArea = {
        x: imageMargins.left,
        y: imageMargins.top,
        width: Math.max(1, sizeConfig.width - imageMargins.left - imageMargins.right),
        height: Math.max(1, sizeConfig.height - imageMargins.top - imageMargins.bottom)
      }

      const textLeftX = imageArea.x + textInset
      const textRightX = imageArea.x + imageArea.width - textInset
      const textTopY = textBandBaseline
      const textBottomY = sizeConfig.height - textBandBaseline
      const dateX = sizeConfig.width - textBandBaseline
